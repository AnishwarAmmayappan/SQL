...
select product from bank_inventory_pricing where month=1 group by product having count(product)>1;
...
select distinct product,quantity,price,estimated_sale_price,purchase_cost from bank_inventory_pricing;
...
select month,sum(purchase_cost) from bank_inventory_pricing where month in (1,2) group by month;

...
select product,avg(estimated_sale_price)+(avg(estimated_sale_price)*(15/100)) as avg
from bank_inventory_pricing 
group by product
having avg(purchase_cost)> avg(estimated_sale_price);

# Question 8:
# 8) Calculate average estimated_sale_price for each product . For any not updated estimated_sale_price, replace the value with purchase_cost 

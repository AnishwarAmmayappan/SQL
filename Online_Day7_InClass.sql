...
select a.customer_id,a.customer_name,avg(b.balance_amount) from bank_customer a
...
select a.customer_id,b.account_number,case when b.balance_amount=0 then c.transaction_amount end as 'bal_amt' from bank_customer a 
inner join bank_account_details b on a.customer_id=b.customer_id
inner join bank_account_transaction c on b.account_number=c.account_number
...
select a.customer_id,b.account_number,b.balance_amount,c.transaction_amount,c.transaction_date
from bank_customer a inner join
bank_account_details b on a.customer_id=b.customer_id
inner join bank_account_transaction c on b.account_number=c.account_number
where monthname(c.transaction_date) in ('march','april') and year(c.transaction_date)=2020;
...
bank_account_details b on a.customer_id=b.customer_id
inner join bank_account_transaction c on b.account_number=c.account_number
where date_format(c.transaction_date,'%Y-%m')!='2020-03';
...
select a.customer_id,a.customer_name,b.balance_amount,b.account_number,c.transaction_date
from bank_customer a 
inner join bank_account_details b on a.customer_id=b.customer_id
inner join bank_account_transaction c on b.account_number=c.account_number
where monthname(c.transaction_date) in ('january','february','march');
...
select a.account_number,b.event,b.customer_message 
from bank_account_details a cross join bank_customer_messages b
where b.event='adhoc' and a.account_type='savings';
...
select a.customer_id,a.account_number,a.account_type, 
case 
when b.transaction_amount like '%-%' then a.balance_amount+b.transaction_amount
else a.balance_amount
end balance_amt from bank_account_details a 
inner join bank_account_transaction b on a.account_number=b.account_number
...
select a.account_number,a.account_number,b.transaction_amount,c.linking_account_number,c.account_type
from bank_account_details a inner join bank_account_transaction b on a.account_number=b.account_number
inner join bank_account_relationship_details c on b.account_number=c.account_number;
...
select a.account_number,a.account_type,b.transaction_amount,c.linking_account_number,c.account_type
...
select b.account_number primary_account,
b.account_type primary_account_type,
c.account_number secondary_account,
c.account_type secondary_account_type
from bank_account_relationship_details b
join bank_account_relationship_details c
on b.account_number = c.linking_account_number
and c.account_type like '%Credit%';

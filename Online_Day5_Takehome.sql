...
select banker,case
when sum(revenue-cost)>0 then sum(revenue - cost) end
from bank_branch_pl group by banker having sum(revenue - cost) is not null;

# Question 2:
# 2)  Find the average Estimated_ sale_price for each product  
/* Solution */

select product,avg(estimated_sale_price) as avg from bank_inventory_pricing group by product;

# Question 3:
# 3)  Find the  month wise total quantity of each product
/* Solution */

select product,month,sum(quantity) from bank_inventory_pricing group by month,product;
...
select product,case
when max(estimated_sale_price - purchase_cost)>0 then round(max(estimated_sale_price - purchase_cost),2)
end as profit
 from bank_inventory_pricing 
 group by product
having case
when max(estimated_sale_price - purchase_cost)>0 then max(estimated_sale_price - purchase_cost) end is not null;

# Question 5:
# 4)  Find the minimum revenue for each banker if estimated_profit is greater than 20000000.
/* Solution */

select banker,min(revenue),estimated_profit from bank_branch_pl
